...
class HAElectricityPriceCard extends HTMLElement {
  ...
  _render() {
    ...
    const tab = (id, name, label, icon) =>
      `<button data-tab="${id}" class="${this._tab === id ? "active" : ""}"><span class="tab-copy"><b>${name}</b><small>${label}</small></span>${this._tab === id ? `<em>AKTIV</em>` : ""}<ha-icon icon="${icon}"></ha-icon></button>`;
